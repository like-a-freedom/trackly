...
pub fn filter_pace_data(
    ...
    if classifications.is_empty() || config.spike_multiplier == get_env_f64("PACE_SPIKE_MULTIPLIER_DEFAULT", 3.0) {
...
fn apply_statistical_filter(
    ...
    let variance = valid_paces
        .iter()
        .map(|p| (p - mean).powi(2))
        .sum::<f64>() / valid_paces.len() as f64;
...
fn apply_smoothing(
    ...
    for i in 0..pace_data.len() {
        if pace_data[i].is_some() {
            ...
            let mut window_values: Vec<f64> = pace_data[start..end]
                .iter()
                .filter_map(|&p| p)
                .collect();
...
mod tests {
    ...
    fn test_statistical_filter() {
        let pace_data = vec![
            Some(5.0), Some(5.2), Some(4.8), Some(5.1), 
            Some(15.0), // Outlier
            Some(5.3), Some(4.9), Some(5.0)
        ];
    ...
    fn test_spike_filter() {
        let pace_data = vec![
            Some(5.0), Some(5.2), Some(15.0), // Spike
            Some(5.1), Some(5.3)
        ];
    ...
    fn test_needs_smoothing() {
        ...
        let stable_data = vec![
            Some(5.0), Some(5.1), Some(4.9), Some(5.2), Some(4.8),
            Some(5.0), Some(5.1), Some(4.9), Some(5.2), Some(4.8),
        ];
        ...
        let noisy_data = vec![
            Some(5.0), Some(8.0), Some(3.0), Some(7.0), Some(2.0),
            Some(6.0), Some(4.0), Some(9.0), Some(1.0), Some(5.0),
        ];
    ...
    fn test_full_filter_pipeline() {
        ...
        let speed_data = vec![
            Some(12.0), Some(24.0), Some(11.5), Some(2.4),
            Some(11.8), Some(12.2), None, Some(11.3),
        ];
        
        let time_diffs = vec![
            Some(10.0), Some(15.0), Some(12.0), Some(11.0),
            Some(13.0), Some(14.0), Some(16.0), Some(11.0),
        ];
        ...
        assert!(filtered[1].is_none(), "Fast pace (1.5 min/km) should be filtered for running");
        assert!(filtered[3].is_none(), "Slow pace (25.0 min/km) should be filtered for running");
        ...
        assert!(valid_count >= 2, "At least some valid values should remain, got {}", valid_count);
    }
  ...
  <trk>
    ...
    <trkseg>
      ...
      <trkpt lat="55.003" lon="37.0">
        ...
        assert!(parsed_data.pace_data.is_some(), "Pace data should be present");
        ...
        for i in 1..pace_data.len() {
            if let Some(pace) = pace_data[i] {
                // Pace should be reasonable (between 1-30 min/km for walking/running)
                assert!(pace >= 1.0 && pace <= 30.0, "Pace {} should be within reasonable range", pace);
            }
        }

        // Verify that speed data is also present and filtered
        assert!(parsed_data.speed_data.is_some(), "Speed data should be present");
        let speed_data = parsed_data.speed_data.unwrap();
        assert_eq!(speed_data.len(), 4);
        
        // Verify classifications were used for filtering
        assert!(!parsed_data.auto_classifications.is_empty(), "Auto classifications should be present");
    }
    ...
    fn test_cycling_vs_running_filtering_differences() {
        ...
        let speed_data = vec![
            Some(30.0), Some(15.0), Some(7.5), Some(2.4),
        ];
        
        let time_diffs = vec![
            Some(10.0), Some(15.0), Some(12.0), Some(10.0),
        ];
        
        // Test with running classification (stricter pace limits: max=15.0, min=2.0)
        let running_classifications = vec![TrackClassification::TempoRun];
        let running_filtered = filter_pace_data(&pace_data, &speed_data, &time_diffs, &running_classifications);
        
        // Test with hiking classification (more lenient pace limits: max=30.0, min=3.0)  
        let hiking_classifications = vec![TrackClassification::Hiking];
        let hiking_filtered = filter_pace_data(&pace_data, &speed_data, &time_diffs, &hiking_classifications);
        ...
        println!("Running valid count: {}, Hiking valid count: {}", running_valid_count, hiking_valid_count);
        ...
        assert_ne!(running_filtered, hiking_filtered, 
        ...
        assert!(running_filtered[3].is_none(), "Running should filter out very slow pace (25.0)");
        assert!(hiking_filtered[0].is_none(), "Hiking should filter out very fast pace (2.0)");
    }
    ...
    fn test_cycling_detection_and_filtering() {
        ...
        let speed_data = vec![
            Some(40.0), Some(30.0), Some(20.0), Some(60.0),
        ];
        
        let time_diffs = vec![
            Some(10.0), Some(15.0), Some(12.0), Some(8.0),
        ];
        
        // Test with no explicit classifications (should detect cycling)
        let no_classifications: Vec<TrackClassification> = vec![];
        let cycling_filtered = filter_pace_data(&pace_data, &speed_data, &time_diffs, &no_classifications);
        ...
        assert!(cycling_valid_count >= running_valid_count, 
    ...
    fn test_cycling_detection_heuristics() {
        ...
        assert!(cycling_config.is_some(), "High speeds should be detected as cycling");
        ...
        assert!(walking_config.is_none(), "Low speeds should not be detected as cycling");
        ...
        assert!(mixed_config.is_some(), "Predominantly fast speeds should be detected as cycling");
    }
}